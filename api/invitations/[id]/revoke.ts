...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const invitationLookup = await withRlsContext(context.currentUser.id, context.currentUser.email, null, (client) =>
      client.query(
    ...
    const revokeResult = await withRlsContext(context.currentUser.id, context.currentUser.email, invitation.organizationId, (client) =>
      client.query(
        `update invitations
         set status = 'revoked'
