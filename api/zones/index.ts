...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === "POST") {
      ...
      const existingZoneResult = await withRlsContext(
        context.currentUser.id,
        activeOrganization.id,
        (client) =>
          client.query(
            "select id from dns_zones where organization_id = $1 and name = $2 limit 1",
            [activeOrganization.id, normalizedName],
          ),
      );
      ...
      try {
        const zoneResult = await withRlsContext(
          context.currentUser.id,
          activeOrganization.id,
          (client) =>
            client.query(
